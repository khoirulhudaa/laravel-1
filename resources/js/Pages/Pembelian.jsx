import { ActionGroup, ApproveButton, DeleteButton, EditButton, RollbackButton } from '@/Components/DataTable/ActionButtons';
...
export default function Pembelian({dataPembelian}) {
    ...
    const confirmDelete = (item) => {
        router.delete(route('pembelian.destroy', item.id), {
            onSuccess: () => {
                onCloseModalDelete();
            }
        })
    ...
    const handleConfirmRollback = (item) => {        
        router.put(route('pembelian.rollbackToPending', item.id), {
            onSuccess: () => {
                setShowModal(false)
                setSelectItem(null)
            },
    ...
    const handleApproval = (item) => {
        router.post(route('pembelian.approval', item.id))
    }
    ...
    const handleEdit = (item) => {
        router.get(route('pembelian.edit', item.id));
    }
    ...
    return (
        ...
        <DataTable 
            ...
            description="Daftar pembelian produk" 
            actions={actions}
            columns={columns}
            prefix={'pembelian'}
        />
